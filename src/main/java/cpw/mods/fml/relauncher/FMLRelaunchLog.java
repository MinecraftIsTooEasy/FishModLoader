...
public class FMLRelaunchLog
{
    ...
    private static void configureLogging()
    {
        try
        {
            ...
            try {
                LogWrapper.retarget((org.apache.logging.log4j.Logger) log.myLog);
            } catch (Throwable __fmlIgnored) {
                // Log4j not available in this classloader context — using reflective bridge instead
            }

            // --- Reflective Log4j bridge: route JUL output through FishModLoader's Log4j logger ---
            boolean log4jBridgeActive = false;
            try {
                final ClassLoader sysLoader = ClassLoader.getSystemClassLoader();
                final Class<?> fishModLoaderClass = sysLoader.loadClass("net.xiaoyu233.fml.FishModLoader");
                final Field logField = fishModLoaderClass.getField("LOGGER");
                final Object log4jLogger = logField.get(null);  // org.apache.logging.log4j.Logger

                // Resolve Log4j Level constants
                final Class<?> l4jLevelClass = sysLoader.loadClass("org.apache.logging.log4j.Level");
                final Object L_ERROR = l4jLevelClass.getField("ERROR").get(null);
                final Object L_WARN  = l4jLevelClass.getField("WARN").get(null);
                final Object L_INFO  = l4jLevelClass.getField("INFO").get(null);
                final Object L_DEBUG = l4jLevelClass.getField("DEBUG").get(null);
                final Object L_TRACE = l4jLevelClass.getField("TRACE").get(null);

                // Log4j Logger.log(Level, String) and Logger.log(Level, String, Throwable)
                final Method logMsg = log4jLogger.getClass().getMethod("log", l4jLevelClass, String.class);
                final Method logMsgThrown = log4jLogger.getClass().getMethod("log", l4jLevelClass, String.class, Throwable.class);

                // JUL Handler that reflectively forwards to Log4j
                Handler bridgeHandler = new Handler() {
                    @Override
                    public void publish(LogRecord record) {
                        int sev = record.getLevel().intValue();
                        Object l4jLevel;
                        if (sev >= Level.SEVERE.intValue())       l4jLevel = L_ERROR;
                        else if (sev >= Level.WARNING.intValue()) l4jLevel = L_WARN;
                        else if (sev >= Level.INFO.intValue())    l4jLevel = L_INFO;
                        else if (sev >= Level.FINE.intValue())    l4jLevel = L_DEBUG;
                        else                                      l4jLevel = L_TRACE;

                        String msg = getFormatter().formatMessage(record);
                        Throwable thrown = record.getThrown();
                        try {
                            if (thrown != null) {
                                logMsgThrown.invoke(log4jLogger, l4jLevel, msg, thrown);
                            } else {
                                logMsg.invoke(log4jLogger, l4jLevel, msg);
                            }
                        } catch (Exception e) {
                            System.err.println("Log4j bridge failed: " + e.getMessage());
                        }
                    }
                    @Override public void flush() {}
                    @Override public void close() {}
                };
                // Bare message format — Log4j adds timestamp/thread
                bridgeHandler.setFormatter(new Formatter() {
                    @Override
                ...
                log.myLog.addHandler(bridgeHandler);
                log.myLog.setUseParentHandlers(false);
                log4jBridgeActive = true;
            } catch (Throwable bridgeFailed) {
                // FishModLoader or Log4j not reachable — fall through to JUL-only
            }
            // --- end reflective bridge ---

            // Always wire up STDOUT/STDERR log hierarchy for stream capture
            ...
            if (!log4jBridgeActive) {
                // JUL-only fallback: console handler + optional file handler
                log.myLog.setUseParentHandlers(false);
                ...
                if (minecraftHome != null && logFileNamePattern != null)
                {
                ...
                if (fileHandler != null)
                {
                    ...
                    if (formatter != null) ConsoleLogThread.wrappedHandler.setFormatter(formatter);
                }
            }

            // Set system out/err to log streams (works in both bridge and JUL-only modes)
            errCache = System.err;
        ...
        {
            // Graceful degradation: logger configuration failed, but log.myLog is still usable
            if (log.myLog != null) {
                log.myLog.log(Level.WARNING, "FMLRelaunchLog configuration failed: " + t.getMessage());
            }
        }
        configured = true;
    }
