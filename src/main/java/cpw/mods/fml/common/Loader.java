...
public class Loader
{
    ...
    private Loader()
    {
        ...
        String actualMCVersion = "1.6.4";
        if (mccversion != null && !mccversion.equals(actualMCVersion))
        {
    ...
    /**
     * Register an externally-constructed Forge mod (from LegacyModLifecycle)
     * with Loader's internal data structures so that FML APIs like
     * {@link #getModList()}, {@link #getReversedModObjectList()},
     * {@link #isModLoaded()} etc. work transparently.
     *
     * <p>This method lazily initializes the mod list and LoadController if
     * {@link #loadMods()} has not been called (which is the norm under
     * FishModLoader). The mod instance is registered in the controller's
    ...
    public static boolean isModLoaded(String modname)
    {
        ...
        if (inst.namedMods == null || inst.modController == null) return false;
        return inst.namedMods.containsKey(modname) && inst.modController.getModState(inst.namedMods.get(modname))!=ModState.DISABLED;
    }
    ...
    {
        if (modController == null) return HashBiMap.create();
    ...
    public LoadController getModController() {
