...
import net.xiaoyu233.fml.reload.transform.forge_compat.accessor.NetLoginHandlerAccessor;
...
public class FMLNetworkHandler
{
    ...
    public static void handlePacket250Packet(Packet250CustomPayload packet, INetworkManager network, NetHandler handler)
    {
        ...
        {
            ((IMixinNetHandler) handler).handleVanilla250Packet(packet);
        }
    ...
    private void handleFMLPacket(Packet250CustomPayload packet, INetworkManager network, NetHandler netHandler)
    {
        ...
        {
            userName = ((NetLoginHandlerAccessor) netHandler).getClientUsername();
        }
        else
        {
            EntityPlayer pl = ((IMixinNetHandler) netHandler).getPlayer();
            if (pl != null)
            {
    ...
    private void handleClientConnection(NetLoginHandler netLoginHandler, MinecraftServer server, SocketAddress address, String userName)
    {
        if (!loginStates.containsKey(netLoginHandler))
        {
            if (handleVanillaLoginKick(netLoginHandler, server, address, userName))
            {
                ...
                ((IMixinNetLoginHandler) netLoginHandler).completeConnection("You don't have FML installed, you cannot connect to this huix.mixins.server");
                return;
            }
            else
            {
        ...
        case LOGIN_RECEIVED:
            ...
            if (modKick != null)
            {
                ((IMixinNetLoginHandler) netLoginHandler).completeConnection(modKick);
            ...
            ((NetLoginHandlerAccessor) netLoginHandler).setField_72544_i(false);
        ...
        case CONNECTION_VALID:
            ((IMixinNetLoginHandler) netLoginHandler).completeConnection(null);
            loginStates.remove(netLoginHandler);
            break;
        case MISSING_MODS_OR_VERSIONS:
            ((IMixinNetLoginHandler) netLoginHandler).completeConnection("The huix.mixins.server requires mods that are absent or out of date on your client");
            loginStates.remove(netLoginHandler);
            break;
        case FML_OUT_OF_DATE:
            ((IMixinNetLoginHandler) netLoginHandler).completeConnection("Your client is not running a new enough version of FML to connect to this huix.mixins.server");
            loginStates.remove(netLoginHandler);
            break;
        default:
            ((IMixinNetLoginHandler) netLoginHandler).completeConnection("There was a problem during FML negotiation");
            loginStates.remove(netLoginHandler);
            break;
        }
    }
    ...
    private boolean handleVanillaLoginKick(NetLoginHandler netLoginHandler, MinecraftServer server, SocketAddress address, String userName)
    {
        ...
        if (kickReason != null)
        {
            ((IMixinNetLoginHandler) netLoginHandler).completeConnection(kickReason);
        }
