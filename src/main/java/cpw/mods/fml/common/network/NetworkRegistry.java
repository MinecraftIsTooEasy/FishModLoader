...
public class NetworkRegistry
{
    ...
    {
        generateChannelRegistration(((IMixinNetHandler) clientHandler).getPlayer(), clientHandler, manager);
        for (IConnectionHandler handler : connectionHandlers)
        {
    ...
    void handleCustomPacket(Packet250CustomPayload packet, INetworkManager network, NetHandler handler)
    {
        ...
        {
            handleRegistrationPacket(packet, (Player) ((IMixinNetHandler) handler).getPlayer());
        }
        else if ("UNREGISTER".equals(packet.channel))
        {
            handleUnregistrationPacket(packet, (Player) ((IMixinNetHandler) handler).getPlayer());
        }
        else
        {
            handlePacket(packet, network, (Player) ((IMixinNetHandler) handler).getPlayer());
        }
    }
...
//        }
    ...
    public void registerGuiHandler(Object mod, IGuiHandler handler)
    {
        ...
        if (mc == null)
        {
            FMLLog.log(Level.WARNING, "Mod %s attempted to register a gui network handler but no mod container was found", mod.getClass().getName());
            return;
        }
