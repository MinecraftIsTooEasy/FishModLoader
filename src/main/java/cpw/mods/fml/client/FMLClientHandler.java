...
import net.xiaoyu233.fml.reload.utils.NetClientHandlerHelper;
 ...
 *
 * The FML entry point is { #beginMinecraftLoading(Minecraft, List)} called from
 * {@link Minecraft}
 *
...
//        TextureFXManager.instance().setClient(client);
    ...
    public void finishMinecraftLoading()
    {
        ...
        ((IMixinSoundManager) client.sndManager).setSoundSystemLoaded(true);
        // Reload resources
...
//            TextureFXManager.instance().loadTextures(client.field_71418_C.func_77292_e());
    ...
    {
        ((IMixinNetClientHandler) handler).fmlPacket131Callback(mapData);
    }
    ...
    public void callbackIdDifferenceResponse(boolean response)
    {
        if (response)
        {
            ...
            ((IMixinMinecraft) (Object) client).continueWorldLoading();
        }
        else
        {
    ...
    public void disconnectIDMismatch(MapDifference<Integer, ItemData> s, NetHandler toKill, INetworkManager mgr)
    {
        ...
        // Stop GuiConnecting
        ((IMixinGuiConnecting) client.currentScreen).forceTermination();
