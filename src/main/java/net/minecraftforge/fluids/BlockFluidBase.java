...
public abstract class BlockFluidBase extends Block implements IFluidBlock
{
    ...
    public BlockFluidBase(int id, Fluid fluid, Material material)
    {
        ...
        this.setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true);
    ...
    public boolean canDisplace(IBlockAccess world, int x, int y, int z)
    {
        ...
        /* MITE: Material.blocksMovement() does not exist
    ...
    public boolean displaceIfPossible(World world, int x, int y, int z)
    {
        ...
        if (displacementIds.containsKey(bId))
        {
            ...
            {
                /* MITE: dropBlockAsItem(World, int, int, int, int, int) does not exist - uses BlockBreakInfo instead
...
//    }
    ...
    public Vec3 getFlowVector(IBlockAccess world, int x, int y, int z)
    {
        ...
        for (int side = 0; side < 4; ++side)
        {
            ...
            {
                /* MITE: Material.blocksMovement() does not exist - use isSolid() as approximation */
                if (!world.getBlockMaterial(x2, y, z2).isSolid())
                {
        ...
        {
            boolean flag =
                Block.isBlockSolid(world, x,     y,     z - 1) ||
                Block.isBlockSolid(world, x,     y,     z + 1) ||
                Block.isBlockSolid(world, x - 1, y,     z    ) ||
                Block.isBlockSolid(world, x + 1, y,     z    ) ||
                Block.isBlockSolid(world, x,     y + 1, z - 1) ||
                Block.isBlockSolid(world, x,     y + 1, z + 1) ||
                Block.isBlockSolid(world, x - 1, y + 1, z    ) ||
                Block.isBlockSolid(world, x + 1, y + 1, z    );
