...
public class BlockEvent extends Event {
    ...
    public static class BreakEvent extends BlockEvent 
    {
        ...
        public BreakEvent(int x, int y, int z, World world, Block block, int blockMetadata, EntityPlayer player)
        {
            ...
            /* MITE: canSilkHarvest takes (int) not (World,EntityPlayer,int,int,int,int); getDamageValue/getExpDrop don't exist */
            if (block == null || /* MITE: player.canHarvestBlock(Block) does not exist */ false || // Handle empty block or player unable to break block scenario
                block.canSilkHarvest(blockMetadata) && EnchantmentHelper.getSilkTouchModifier(player)) // If the block is being silk harvested, the exp dropped is 0
            {
                this.exp = 0;
            }
            else
            {
                /* MITE: getDamageValue/getExpDrop not available
